...
X = np.concatenate((X2All , X3All ))
y = np.concatenate((y2All , y3All ))
...
########### predict for display ################
pre2 = forest.predict(X2All)
pre3 = forest.predict(X3All)
...
for i in range(len(pre2)):
    wt.writerow([p2[i] , pre2[i] , y2All[i]])

wt.writerow(["Sample4"])
for i in range(len(pre3)):
    wt.writerow([p3[i] , pre3[i] , y3All[i]])
...
s2 = plt.scatter(pre3Train , y3Train , c = 'b' , alpha = 0.5 , label = "#4Train")
s3 = plt.scatter(pre2Test , y2Test   , c = 'g' , alpha = 0.9 , label = "#2Test")
s4 = plt.scatter(pre3Test , y3Test   , c = 'y' , alpha = 0.9 , label = "#4Test")
